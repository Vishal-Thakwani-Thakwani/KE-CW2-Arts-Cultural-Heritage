...
Our custom namespace (`cah:`) introduces 21 classes and over 30 object/datatype properties. Subclass extensions of CIDOC-CRM include `cah:Artist rdfs:subClassOf crm:E21_Person`, `cah:Artwork rdfs:subClassOf crm:E22_Man-Made_Object`, `cah:Exhibition rdfs:subClassOf crm:E7_Activity`, `cah:RestorationEvent rdfs:subClassOf crm:E11_Modification`, and `cah:ProvenanceEvent rdfs:subClassOf crm:E10_Transfer_of_Custody`. Subclass extensions of EDM include `cah:Artwork rdfs:subClassOf edm:ProvidedCHO`. Property alignments include `cah:createdBy` as a shortcut property abstracting the CIDOC-CRM event path (`E22_Man-Made_Object` → `P108i_was_produced_by` → `E12_Production` → `P14_carried_out_by` → `E21_Person`), `cah:heldBy` aligned to `crm:P50_has_current_keeper`, and `cah:locatedIn` aligned to `crm:P53_has_former_or_current_location`. Complex relational axioms were added in a second LLM-assisted pass, including cardinality constraints (many-to-many for Artist–Artwork), asymmetry (`owl:AsymmetricProperty` on Artwork `createdInPeriod` ArtPeriod), and five inverse property pairs.
...
*Quality.* All 20 SPARQL queries were executed against the populated KG. Of these, 16 out of 20 returned non-empty, well-formed results. The 4 queries that returned empty results (CQ3, CQ12, CQ13, CQ15) do so because of instance coverage gaps — no RestorationEvent, ProvenanceEvent, or Curator instances exist in the current KG — rather than query logic errors. The competency questions themselves are not replaced; the unanswered ones are discussed as KG coverage limitations in the Completion Analysis. The structured nature of the KG ensures answers are verifiable and consistently reproducible, unlike single-prompt LLM responses which lack provenance and can hallucinate temporal or geographic facts.
